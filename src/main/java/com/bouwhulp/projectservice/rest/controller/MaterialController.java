...
public class MaterialController {
    ...
    public ResponseEntity<MaterialDto> postMaterial(@RequestBody MaterialDto materialDto) {
        MaterialEntity materialEntity = materialRepository.saveAndFlush(materialDto.toEntity());

        return ResponseEntity.ok(materialEntity.toDto());
    }
    ...
    public ResponseEntity<MaterialDto> getMaterial(@PathVariable UUID materialId) {
        ...
        return optionalMaterialDto
                .map(materialEntity -> ResponseEntity.ok(materialEntity.toDto()))
